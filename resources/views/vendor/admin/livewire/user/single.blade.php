<tr x-data="{ modalIsOpen : false }">
    ...
    @if(getCrudConfig('User')->delete or getCrudConfig('User')->update)
        <td>
            ...
            @if(getCrudConfig('User')->update && hasPermission(getRouteName().'.users.update', 1, 1, $user))
                <a href="@route(getRouteName().'.users.update', $user->id)" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('User')->delete && hasPermission(getRouteName().'.users.delete', 1, 1, $user))
