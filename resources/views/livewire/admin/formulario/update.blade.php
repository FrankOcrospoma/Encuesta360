<div class="card">
    ...
    <form action="{{ route('formularios.update', ['formulario' => $formulario->id]) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="card-body">
            ...
            <div class='form-group'>
                ...
                <input type='text' id='input-nombre' name='nombre' value="{{ old('nombre', $formulario->nombre) }}" class="form-control @error('nombre') is-invalid @enderror" placeholder='' autocomplete='on'>
                ...
                <div class="d-flex" style="align-items: stretch;">
                    <select name='preguntas' id='input-preguntas' class="form-control flex-grow-1" style="height: 38px;">
                        @foreach($preguntas as $pregunta)
                            <option value='{{ $pregunta->id }}' data-categoria='{{ $pregunta->Categoria }}' data-estado='{{ $pregunta->estado ? "true" : "false" }}'
                              > 
                                {{ $pregunta->texto }}
                            </option>
                        @endforeach
                    ...
                    @foreach(App\Models\Respuesta::where('estado', 1)->get() as $respuesta)
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="respuestas[]" value="{{ $respuesta->id }}" 
                                @if(in_array($respuesta->id, $respuestasSeleccionadas)) checked @endif>
        ...
        <div class="card-footer">
            <button type="submit" class="btn btn-info">Actualizar Formulario</button>
...
<script>
        // Agregar evento de carga al documento
        document.addEventListener('DOMContentLoaded', function() {
        // Agregar eventos de "quitar pregunta" a las preguntas que ya están en la lista
        document.querySelectorAll('.quitar-pregunta').forEach(function(button) {
